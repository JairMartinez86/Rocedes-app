...
export class FabricOunceComponent implements OnInit {
  ...
  displayedColumns: string[] = ["IdOunce", "Ounce", "Category", "FeedDog", "Caliber",  "Editar", "Eliminar"];
  ...
  constructor(private _liveAnnouncer: LiveAnnouncer, private dialog : MatDialog, private _OperacionesService : OperacionesService) { 
    ...
    this.val.add("txt_Operacion_ounce_feed_dog", "1", "LEN>", "0");
    this.val.add("txt_Operacion_ounce_caliber", "1", "LEN>", "0");
  ...
  Limpiar(): void
  {
    ...
    this.val.ValForm.get("txt_Operacion_ounce_feed_dog")?.disable();
    this.val.ValForm.get("txt_Operacion_ounce_caliber")?.disable();
  ...
  onKeyEnter(event: any) : void
  {
    ...
    switch(_input){
      ...
      case "txt_Operacion_ounce_category":
        document?.getElementById("txt_Operacion_ounce_feed_dog")?.focus();
        break;

      case "txt_Operacion_ounce_feed_dog":
        document?.getElementById("txt_Operacion_ounce_caliber")?.focus();
        break;

      case "txt_Operacion_ounce_caliber":
        this.Guardar();
        break;
    }
  ...
  clickRow(row : any, str_Evento : string){
    ...
    if(str_Evento == "Editar")
    {
      ...
      this.val.ValForm.get("txt_Operacion_ounce_feed_dog")?.setValue(row.FeedDog);
      this.val.ValForm.get("txt_Operacion_ounce_caliber")?.setValue(row.Caliber);
  ...
  Nuevo() :void
  {
    ...
    this.val.ValForm.get("txt_Operacion_ounce_feed_dog")?.enable();
    this.val.ValForm.get("txt_Operacion_ounce_caliber")?.enable();
  ...
  Guardar() : void
  {
    ...
    datos.FeedDog = String(this.val.ValForm.get("txt_Operacion_ounce_feed_dog")?.value).trimEnd();
    datos.Caliber = String(this.val.ValForm.get("txt_Operacion_ounce_caliber")?.value).trimEnd();
    datos.Evento = "Nuevo";
      ...
      _dialog.afterClosed().subscribe(s =>{
        if(_json["esError"] == 0)
        {
          ...
          if(index >= 0)
          {
            ...
            ELEMENT_DATA_OUNCE[index].FeedDog = _json["d"].FeedDog;
            ELEMENT_DATA_OUNCE[index].Ounce = _json["d"].Ounce;
            ELEMENT_DATA_OUNCE[index].Caliber = _json["d"].Caliber;
          }
          else
          {
