...
export class ProductoComponent implements OnInit {
  ...
  displayedColumns: string[] = ["IdProducto", "Nombre", "LevelOfComplexity",  "Editar", "Eliminar"];
  ...
  constructor(private _liveAnnouncer: LiveAnnouncer, private dialog : MatDialog, private _ProductoService : ProductoService) { 
    ...
    this.val.add("txt_inv_producto_level", "1", "LEN>", "0");
    this.val.add("txt_inv_producto_level", "2", "NUM>", "0");
  }
  ...
  Limpiar(): void
  {
    ...
    this.val.ValForm.get("txt_inv_producto_level")?.disable();
  ...
  onKeyEnter(event: any) : void
  {
    ...
    switch(_input){
      ...
      case "txt_inv_producto_nombre":
        document?.getElementById("txt_inv_producto_level")?.focus();
        break;

      case "txt_inv_producto_level":
        this.Guardar();
        break;
    }
  ...
  clickRow(row : any, str_Evento : string){
    ...
    if(str_Evento == "Editar")
    {
      ...
      this.val.ValForm.get("txt_inv_producto_level")?.setValue(row.LevelOfComplexity);
  ...
  Nuevo() :void
  {
    ...
    this.val.ValForm.get("txt_inv_producto_level")?.enable();
  ...
  Guardar() : void
  {
    ...
    datos.LevelOfComplexity = Number(this.val.ValForm.get("txt_inv_producto_level")?.value)
    datos.Evento = "Nuevo";
     ...
     let _dialog =  this.dialog.open(DialogoComponent, {
      ...
      _dialog.afterClosed().subscribe(s =>{
        if(_json["esError"] == 0)
        {
          ...
          if(index >= 0)
          {
            ELEMENT_DATA_PRODUCTO[index].LevelOfComplexity = _json["d"].LevelOfComplexity;
